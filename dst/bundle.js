...
var require_browser_request = __commonJS((exports2, module2) => {
  (function(root, factory) {
    ...
    function run_xhr(options) {
      ...
      function on_state_change(event) {
        ...
        request2.log.debug("State change", {state: xhr.readyState, id: xhr.id, timed_out});
      ...
      function emitDraw(env, outer, inner, args) {
        ...
        function emitElements() {
          ...
          if (defn) {
            ...
            ELEMENTS2 = scope.def();
            scope("if(", shared.vao, ".currentVAO){", ELEMENTS2, "=", env.shared.elements + ".getElements(" + shared.vao, ".currentVAO.elements);", !extVertexArrays ? "if(" + ELEMENTS2 + ")" + GL + ".bindBuffer(" + GL_ELEMENT_ARRAY_BUFFER$2 + "," + ELEMENTS2 + ".buffer.buffer);" : "", "}else{", ELEMENTS2, "=", DRAW_STATE, ".", S_ELEMENTS, ";", "if(", ELEMENTS2, "){", GL, ".bindBuffer(", GL_ELEMENT_ARRAY_BUFFER$2, ",", ELEMENTS2, ".buffer.buffer);}}");
          }
          return ELEMENTS2;
        }
        function emitCount() {
...
// src/fluid-shaders/arrows/arrows.fs
var require_arrows2 = __commonJS((exports2, module2) => {
  module2.exports = "precision highp float;\n\nvarying float vMagnitude;\n\nuniform vec4 uColor;\n\nvoid main ()\n{\n  gl_FragColor = uColor;\n}\n";
});

// src/fluid-shaders/pressure/rgb-exponential.fs
var require_rgb_exponential = __commonJS((exports2, module2) => {
...
// src/fluid-shaders/add-force.fs
var require_add_force = __commonJS((exports2, module2) => {
...
// src/fluid-shaders/advect.fs
var require_advect = __commonJS((exports2, module2) => {
...
// src/fluid-shaders/divergence.fs
var require_divergence = __commonJS((exports2, module2) => {
  module2.exports = "precision mediump float;\nprecision mediump sampler2D;\n\nvarying highp vec2 vUv;\nvarying highp vec2 vL;\nvarying highp vec2 vR;\nvarying highp vec2 vT;\nvarying highp vec2 vB;\n\nuniform sampler2D uVelocity;\nuniform highp vec2 uTexelSize;\nuniform float rho;\nuniform float dt_inv;\n\nvoid main ()\n{\n  float L = texture2D(uVelocity, vL).x;\n  float R = texture2D(uVelocity, vR).x;\n  float T = texture2D(uVelocity, vT).y;\n  float B = texture2D(uVelocity, vB).y;\n\n  vec2 C = texture2D(uVelocity, vUv).xy;\n\n  if (vL.x < 0.0) { L = -C.x; }\n  if (vR.x > 1.0) { L = -C.x; }\n  if (vT.y > 1.0) { L = -C.y; }\n  if (vB.y < 0.0) { L = -C.y; }\n\n  float div = 0.5 * (R - L + T - B);\n  gl_FragColor = vec4(div, 0.0, 0.0, 1.0);\n}\n";
});

// src/fluid-shaders/pressure-iteration.fs
var require_pressure_iteration = __commonJS((exports2, module2) => {
  module2.exports = "precision mediump float;\nprecision mediump sampler2D;\n\nvarying highp vec2 vUv;\nvarying highp vec2 vL;\nvarying highp vec2 vR;\nvarying highp vec2 vT;\nvarying highp vec2 vB;\n\nuniform sampler2D uDivergence;\nuniform sampler2D uPressure;\nuniform float dt;\nuniform float dissipation;\n\nvoid main ()\n{\n  float L = texture2D(uPressure, vL).x;\n  float R = texture2D(uPressure, vR).x;\n  float T = texture2D(uPressure, vT).x;\n  float B = texture2D(uPressure, vB).x;\n\n  float divergence = texture2D(uDivergence, vUv).x;\n  float pressure = (L + R + B + T - divergence) * 0.25;\n  pressure = pressure / (1.0 + dissipation * dt);\n\n  gl_FragColor = vec4(pressure, 0.0, 0.0, 1.0);\n}\n";
});

// src/fluid-shaders/gradient.fs
var require_gradient = __commonJS((exports2, module2) => {
  module2.exports = "precision mediump float;\nprecision mediump sampler2D;\n\nvarying highp vec2 vUv;\nvarying highp vec2 vL;\nvarying highp vec2 vR;\nvarying highp vec2 vT;\nvarying highp vec2 vB;\n\nuniform sampler2D uVelocity;\nuniform sampler2D uPressure;\n\nvoid main ()\n{\n  float L = texture2D(uPressure, vL).x;\n  float R = texture2D(uPressure, vR).x;\n  float T = texture2D(uPressure, vT).x;\n  float B = texture2D(uPressure, vB).x;\n\n  vec2 velocity = texture2D(uVelocity, vUv).xy;\n  velocity.xy -= vec2(R - L, T - B);\n  gl_FragColor = vec4(velocity, 0.0, 1.0);\n}\n";
});

// src/fluid-shaders/draw-buffer.fs
var require_draw_buffer = __commonJS((exports2, module2) => {
  module2.exports = "precision highp float;\nprecision highp	sampler2D;\n\nvarying vec2 vUv;\n\nuniform sampler2D uSource;\n\nvoid main()\n{\n  vec4 uv = texture2D(uSource, vUv);\n\n  gl_FragColor = clamp(uv, 0.0, 1.0);\n}\n";
});

// src/fluid-shaders/draw-regions.fs
var require_draw_regions = __commonJS((exports2, module2) => {
  module2.exports = "precision highp float;\nprecision highp	sampler2D;\n\nvarying vec2 vUv;\n\nuniform sampler2D uSource;\nuniform vec2 uTexelSize;\n\n#define THRESHOLD 1.6\n#define CLAMPED_COLORS\n\nvoid main()\n{\n  vec4 uv = texture2D(uSource, vUv);\n\n#ifndef CLAMPED_COLORS\n  uv = clamp(uv, 0.0, 1.0);\n#endif\n\n  if (length(uv) > THRESHOLD)\n  {\n    gl_FragColor = vec4(1.0);\n  }\n  else\n  {\n    gl_FragColor = vec4(vec3(0.0), 1.0);\n  }\n}\n";
});

// src/data/glyphs/A_-forces.json
var require_A_forces = __commonJS((exports2, module2) => {
  module2.exports = {
    forces: [
      {
        radius: 1e-3,
...
var require_fluid = __commonJS(() => {
  ...
  var request2 = require_browser_request();
  var COLOR_RESOLUTION = 2048;
  var SIM_RESOLUTION = 512;
  var COLOR_TEXEL_SIZE = 1 / COLOR_RESOLUTION;
  var SIM_TEXEL_SIZE = 1 / SIM_RESOLUTION;
  var CAPTURE_FPS = 30;
  var PRESSURE_JACOBI_ITERATIONS = 50;
  var VELOCITY_GRID_DIVISIONS = 65;
  var parameters = {
    dt: 2,
    velocity: {
      dissipation: 0.5,
  ...
  function create_arrow_geometry() {
    ...
    let elements = [];
    let uvs = [];
    let offsets = [[0, -5e-3], [0.03, 0], [0, 5e-3]];
    ...
    for (var u = 0; u < VELOCITY_GRID_DIVISIONS; u++) {
      for (var v = 0; v < VELOCITY_GRID_DIVISIONS; v++) {
        ...
        let u_screen = 2 * division * u - 1 + division / 2;
        let v_screen = 2 * division * v - 1 + division / 2;
  ...
  var arrows = create_arrow_geometry();
  var color_buffer = new DoubleFramebuffer(COLOR_RESOLUTION);
  var velocity_buffer = new DoubleFramebuffer(SIM_RESOLUTION);
  var pressure_buffer = new DoubleFramebuffer(SIM_RESOLUTION);
  var velocity_emitter_buffer = new DoubleFramebuffer(SIM_RESOLUTION);
  var divergence_buffer = create_buffer(SIM_RESOLUTION);
  var color_picker_buffer = create_buffer(COLOR_RESOLUTION);
  var create_color_buffer = regl({
    framebuffer: regl.prop("target"),
    vert: require_simple(),
    frag: require_half_tone(),
    attributes: {
  ...
  var draw_velocity_field = regl({
    framebuffer: regl.prop("target"),
    vert: require_arrows(),
    frag: require_arrows2(),
    attributes: {
  ...
  var draw_pressure_field = regl({
    framebuffer: regl.prop("target"),
    vert: require_simple(),
    frag: require_rgb_exponential(),
    attributes: {
      aPosition: [-1, -1, -1, 1, 1, 1, 1, -1]
    },
    elements: [0, 1, 2, 0, 2, 3],
  ...
  var create_velocity_buffer = regl({
    framebuffer: regl.prop("target"),
    vert: require_simple(),
    frag: require_d_field(),
    attributes: {
      aPosition: [-1, -1, -1, 1, 1, 1, 1, -1]
    },
    elements: [0, 1, 2, 0, 2, 3],
  ...
  var add_directed_force = regl({
    framebuffer: regl.prop("target"),
    vert: require_simple(),
    frag: require_add_force(),
    attributes: {
      aPosition: [-1, -1, -1, 1, 1, 1, 1, -1]
    },
    elements: [0, 1, 2, 0, 2, 3],
  ...
  var calculate_divergence = regl({
    framebuffer: regl.prop("target"),
    vert: require_simple(),
    frag: require_divergence(),
    attributes: {
      aPosition: [-1, -1, -1, 1, 1, 1, 1, -1]
    },
    elements: [0, 1, 2, 0, 2, 3],
  ...
  var calculate_pressure = regl({
    framebuffer: regl.prop("target"),
    vert: require_simple(),
    frag: require_pressure_iteration(),
    attributes: {
      aPosition: [-1, -1, -1, 1, 1, 1, 1, -1]
    },
    elements: [0, 1, 2, 0, 2, 3],
  ...
  var calculate_velocity_gradient_for_pressure = regl({
    framebuffer: regl.prop("target"),
    vert: require_simple(),
    frag: require_gradient(),
    attributes: {
      aPosition: [-1, -1, -1, 1, 1, 1, 1, -1]
    },
    elements: [0, 1, 2, 0, 2, 3],
  ...
  var draw_buffer = regl({
    framebuffer: regl.prop("target"),
    vert: require_simple(),
    frag: require_draw_buffer(),
    attributes: {
      aPosition: [-1, -1, -1, 1, 1, 1, 1, -1]
    },
    elements: [0, 1, 2, 0, 2, 3],
  ...
  var data = require_A_forces();
  var canvas = document.getElementsByTagName("canvas")[0];
  var timer = null;
  var stream_chunks = [];
  var stream = canvas.captureStream(CAPTURE_FPS);
  ...
  clear_buffer({target: color_buffer.front, clearcolor: [0, 0, 0, 1]});
  clear_buffer({target: velocity_buffer.front, clearcolor: [0, 0, 0, 1]});
  ...
  regl.frame(() => {
    ...
    if (state.render == RENDER_EDGES)
      draw_edges({source: color_buffer.front, target: null});
    if (state.render == RENDER_COLOR && keystate(" "))
      draw_edges({source: color_buffer.front, target: null});
    if (state.render == RENDER_COLOR && !keystate(" "))
      draw_buffer({source: color_buffer.front, target: null});
    if (state.render == RENDER_VELOCITY)
      draw_velocity_field({velocity: velocity_buffer.front, target: null, color: [1, 1, 1, 1], scale: 1});
    if (state.render == RENDER_EMITTER_FIELD)
      draw_velocity_field({velocity: velocity_emitter_buffer.front, target: null, color: [1, 0, 0.1, 1], scale: 25});
    if (state.render == RENDER_PRESSURE)
      draw_pressure_field({pressure: pressure_buffer.front, target: null});
    if (state.render == RENDER_COLOR_PICKER)
      draw_buffer({source: color_picker_buffer, target: null});
    if (state.render == RENDER_RADIUS_PICKER)
      draw_radius_picker({radius: parameters.force.radius * 10, target: null});
    if (state.capture) {
      let a = document.createElement("a");
    ...
    if (state.reset_forces.length > 0) {
      clear_buffer({target: velocity_emitter_buffer.front, clearcolor: [0, 0, 0, 1]});
    }
...
// src/renderstates.js
var RENDER_EDGES = "edges";
var RENDER_COLOR = "color";
var RENDER_PRESSURE = "pressure";
var RENDER_VELOCITY = "velocity";
var RENDER_COLOR_PICKER = "color_picker";
var RENDER_RADIUS_PICKER = "radius_picker";
var RENDER_EMITTER_FIELD = "emitter_field";

// src/ampersand-charset.js
var charset = {
  "113": "q",
  "119": "w",
  "101": "e",
...
// src/inputs.js
var request = require_browser_request();
var event_buffer = [];
var mouse_buffer = [];
var keys = {};
...
var actions = {
  ...
  "3": (parameters, state) => {
    if (state.interacting) {
      state.render = RENDER_EMITTER_FIELD;
...
function handle_events(parameters, state) {
  event_buffer.forEach((event) => {
    if (typeof actions[event.data] !== "undefined") {
      actions[event.data](parameters, state);
    } else if (typeof charset[event.code] !== "undefined" && state.interacting && String.fromCharCode(event.code) == event.data) {
      let glyphname = charset[event.code];
      let target = (false ? "/" : "") + `src/data/ampersands/${glyphname}-forces.json`;
    ...
    } else if (event.type == "mouse" && !keys["Shift"] && state.interacting) {
      if (state.render == RENDER_COLOR_PICKER) {
        state.reset_colors.push(event);
      } else if (state.render == RENDER_RADIUS_PICKER) {
        let radius = Math.sqrt(Math.pow(event.data.pos.x - 0.5, 2) + Math.pow(event.data.pos.y - 0.5, 2)) / 10;
        parameters.force.radius = radius;
        parameters.ink.radius = radius;
      } else {
        state.added_colors.push(event);
      }
    } else if (event.type == "mouse" && keys["Shift"] && typeof event.data.dir !== "undefined" && state.interacting) {
      state.added_forces.push(event);
    }
  });
  event_buffer = [];
}
function register_event_sources() {
  ...
  document.addEventListener("mousedown", (event) => {
    keys["mouse"] = true;
    let uAspectOffset = window.innerWidth / (2 * window.innerHeight) - 0.5;
    let mouse = {
